...
check_ngrok() {
  if ! command -v ngrok &>/dev/null; then
    ...
    echo ""
    echo "After installing, authenticate with your free ngrok account:"
    echo "  ngrok config add-authtoken YOUR_TOKEN"
    echo "  (get your token at: https://dashboard.ngrok.com/authtokens)"
    echo ""
  ...
  if [[ ! -f "$ngrok_config" ]] || ! grep -q "authtoken" "$ngrok_config" 2>/dev/null; then
    ...
    if [[ ! -f "$legacy_config" ]] || ! grep -q "authtoken" "$legacy_config" 2>/dev/null; then
      ...
      echo ""
      echo "If the tunnel fails to start, run:"
      echo "  ngrok config add-authtoken YOUR_TOKEN"
      echo "  (get your token at: https://dashboard.ngrok.com/authtokens)"
      echo ""
    fi
